...
def log_dir():
    #log_dir = input_base_log_dir + "/unsup-" + input_train_prefix.split("/")[-2]
    #log_dir += "/{model:s}_{model_size:s}_{lr:0.6f}/".format(
            #model=input_model,
            #model_size=input_model_size,
            #lr=input_learning_rate)
    log_dir = input_base_log_dir + "/LIVE_2007_2019_daily_returns_embeddings"
...
def train(train_data, prefix, test_data=None, input_batch_size=50, input_model='graphsage_mean',
    ...
    if input_save_embeddings:
        ...
        if input_model == "n2v":
            ...
            save_val_embeddings(sess, model, minibatch, input_validate_batch_size, log_dir(), mod="-test", prefix_name=input_train_prefix)
